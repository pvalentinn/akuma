    ...
    render() {
        ...
        return (
            <View style={styles.div}>
                <TouchableOpacity style={styles.manga} onPress={async () => this.props.showManga(await searchManga(mangas[0].name, this.props.navigation))}>
                    <Image source={{uri: mangas[0].src}} style={styles.img}></Image>
                    <View style={styles.divText}>
                        <Text style={styles.text}>{mangas[0].name}</Text>
                    </View>
                </TouchableOpacity>
                <TouchableOpacity style={styles.manga} onPress={async () => this.props.showManga(await searchManga(mangas[1].name, this.props.navigation))}>
                    <Image source={{uri: mangas[1].src}} style={styles.img}></Image>
                    <View style={styles.divText}>
                        <Text style={styles.text}>{mangas[1].name}</Text>
                    </View>
                </TouchableOpacity>
                <TouchableOpacity style={styles.manga} onPress={async () => this.props.showManga(await searchManga(mangas[2].name, this.props.navigation))}>
